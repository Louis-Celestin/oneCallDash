...
<div class="card my-2">
    <div class="card-body mx-auto">
        <form action="{{ route('stat.tickets_post_general') }}" method="post">
            @csrf
 ...
 {{-- <div class="col-lg-4 col-6">
    ...
    <div class="small-box bg-success" style="color: black !important;">
      <div class="inner">
        ...
        <h3>{{number_format($stat_ticket->where('is_fret', 0)->sum('prix'), 0 , '', ' ')}}</h3>
  ...
  <div class="col-lg-4 col-6">
    ...
    <div class="small-box bg-warning" style="color: black !important;">
      <div class="inner">
        ...
        <h3>{{number_format($stat_ticket->where('is_fret', 1)->sum('prix'), 0 , '', ' ')}}</h3>
  ...
  <div class="col-lg-4 col-6">
    ...
    <div class="small-box bg-danger" style="color: black !important;">
      <div class="inner">
        ...
        <h3>{{number_format($stat_ticket->where('is_fret', 0)->sum('prix') + $stat_ticket->where('is_fret', 1)->sum('prix'), 0 , '', ' ')}}</h3>
        ...
        <div class="card card-zo">
            ...
            <div class="card-body">
                <div class="row">
                  ...
                  <div class="col-md-8">
                      <h6  style="font-weight: bold;">NOMBRE DE TICKET </h6>
                      <h5>{{$stat_ticket->sum('qte_impression'). ""}}</h5>
          ...
          <div class="card-body">
              <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    <h5>{{number_format($stat_ticket->sum('prix'), 0, '', ' ')}}</h5>
    ...
    <div class="card">
      <div class="card-header">
        <div class="d-flex justify-content-between my-2">
          <h3 class="card-title">RAPPORT GENERAL DES TICKETS</h3>
      ...
      <div class="card-body">
        <table id="example3" class="table table-bordered table-striped">
          <thead>
            <tr>
                <th>Gare </th>
                {{-- <th>Code <sup>Tickets</sup> </th> --}}
                <th>Agent <sup>Impression</sup> </th>
                <th>Qte <sup>Tickets</sup></th>
                <th>Montant <sup>CFA</sup></th>
            </tr>
          </thead>
          <tbody>
        @foreach ($stat_ticket as $data)
            <tr>
                <td>{{ $data->nom_gare }}</td>
                {{-- <td>{{ $data->code }}</td> --}}
                <td>{{ $data->name }}</td>
                <td>{{ $data->qte_ticket }}</td>
                <td>{{ $data->prix }}</td>
            </tr>
        @endforeach
          </tbody>
