      ...
      <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            @if ($item->prix * 7 /100 != $soldes->where('date', $item->date)->first()->montant)
                
            <tr>
                <td>{{$loop->index + 1}}</td>
                <td>{{$soldes->where('date', $item->date)->first()->id}}</td>
                <td>{{$item->date}}</td>
                <td>
                  {{$gare->nom_gare}}
                  {{-- @if ($gares->where('gare_id', $item->gare_id)->first() != null)
                      {{$gares->where('gare_id', $item->gare_id)->first()->nom_gare}}
                  @endif --}}
                </td>
                <td>{{$item->prix}}</td>
                <td>{{$item->prix * 7 /100}}</td>
                <td>
                    @if ($soldes->where('date', $item->date)->first() != null)
                        {{$soldes->where('date', $item->date)->first()->montant}}
                    @endif
                </td>
            </tr>
            @endif
