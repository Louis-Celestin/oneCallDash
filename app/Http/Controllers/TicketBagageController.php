...
class TicketBagageController extends Controller
{
    ...
    function InfoBagage()
    {
       // dd(session()->has("bagage"));
        ...
        session(['Infobagagepayes' => $bagage]);
    ...
    public function BagageImpayes(string $statut,Request $request)
    {
       // dd($statut);

        $req=Bagage::where('compagnie_id',Auth::user()->compagnie_id);
        if($statut=='impaye'){
          //  $status='impayés';
            $req->where('is_solded', 0); 
        }
        if($statut=='paye')
        { //$status='payés';
            $req->where('is_solded', 1);
        }

        if(Auth::user()->usertype != 'admin')
        { $req->where('gare_id',Auth::user()->gare_id);}
        ...
        return view('chefdegare.ticket-impaye',compact('Bagage','statut'));
    }
}
