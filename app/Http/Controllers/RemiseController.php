...
//dd($remises);
        ...
        ->whereDate('remises.created_at', date('Y-m-d'))
        ->where('remises.compagnie_id', Auth::user()->compagnie_id)
        ->select(DB::raw('sum(bagage.nbr_de_bagage) as qte'), 'users.id', 'gare.nom_gare',  DB::raw('sum(remises.montant) as montant_remises'))->groupBY('users.id', 'gare.nom_gare')->orderBy('montant_remises', 'DESC')->get('users.name');
    ...
    function filtre(REQUEST $request)
    {
        ...
        $remisesParAgent = $req2->select(DB::raw('sum(bagage.nbr_de_bagage) as qte'), 'users.id', 'gare.nom_gare' , DB::raw('sum(remises.montant) as montant_remises'))->groupBY('users.id', 'gare.nom_gare')->orderBy('montant_remises', 'DESC')->get();
